...
calc_prefactor <- function(n, numRep, r){
  ...
  for(i in 1:numRep){
    ...
    test_f <- simUltra(a,b = a - r, cloneAge  = 100,n = n,nTrees = 1,precBits = 1000,addStem = FALSE,nCores = 1)
  ...
  v1 = ew_CH_f
  alpha_opt_CH_f <- sum(v1 * y1) / sum(v1^2) # pre factor

  quantile_1 <- quantile(ew_CH, probs = 0.05) # 0.05 Quantile
  quantile_3 <- quantile(ew_CH, probs = 0.95) # 0.95 Quantile
  q1 = quantile_1/mean(ew_CH)
  q3 = quantile_3/mean(ew_CH)
  
  return(list(alpha_opt_CH,alpha_opt_CH_f, q1, q3, ew_CH_f, ew_CH))
}

test_p = calc_prefactor(7, 10, 0.5)
print(test_p)

rep_calc_prefactor <- function(n_range, num_Rep, t, r){
  res_alpha_CH = rep(0, length(n_range))
  res_alpha_durrett = rep(0, length(n_range))
  res_alpha_JS = rep(0, length(n_range))
  res_alpha_NL = rep(0, length(n_range))
  
  i = 1
  for(n in n_range){
    res_alpha_temp = calc_prefactor(n, num_Rep, t, r)
    res_alpha_CH[i] = res_alpha_temp[[1]]
    res_alpha_durrett[i] = res_alpha_temp[[2]]
    res_alpha_JS[i] = res_alpha_temp[[3]]
    res_alpha_NL[i] = res_alpha_temp[[4]]
    i = i+1
  }
  return(list(res_alpha_CH, res_alpha_durrett, res_alpha_JS, res_alpha_NL))
}

res_alpha_all_40 = rep_calc_prefactor(c(5, 10), 1000, 40, 2)

res_alpha_all = rep_calc_prefactor(c(5, 6, 7, 8, 9, 10), 1000, 100, 2)
print(res_alpha_all)
# Originale Liste
my_list <- list(
...
write.table(df, "prefactors.txt", sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
